...
| **Git Bash `ln -s` silently deep-copies instead of linking (Developer Mode OFF)** | `vscode/extensions/ritemark` becomes a 534 MB / 32k-file copy that looks right in `ls`, decouples the build from source edits, and fails the pre-commit `[[ -L ]]` invariant | Create the link with `cmd /c mklink /J` (no admin rights needed; Git Bash then reports it as a symlink) and assert `[ -L ]` afterwards. Compare RESOLVED paths, never raw `readlink` — a junction always reads back absolute. Both live in `scripts/lib/extension-link.sh`. |
| **`python3` not always on Windows PATH; only `python`, and that may be a Microsoft Store alias stub** | `command not found` mid-script, or `Python was not found; ... Microsoft Store` and exit 49 | Source `scripts/lib/resolve-python.sh` and use `ritemark_resolve_python`. It skips `WindowsApps` stubs, proves the candidate actually executes, and falls back to the `py -3` launcher. Never probe with `command -v` alone. |
