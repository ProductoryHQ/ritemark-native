...
- Final code review
- Run `node ./scripts/worktree-hygiene.mjs --check` after merge/close and report the `REVIEW` entries; `--clean` is human-authorized (Jarmo says go after reading the report) — never run it yourself
