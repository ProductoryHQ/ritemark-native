...
**Clean-room release invariant (2026-09-02):** a shell RC is built only in a new worktree created by `./scripts/create-release-worktree.sh`: detached at the exact `origin/main` commit, physical pristine VS Code submodule at the recorded gitlink, physical dependency roots installed from lockfiles, and empty target output. `build-prod.sh`, signing, and DMG packaging hard-block unsafe source or missing/mismatched embedded provenance. Development worktrees and locally patched/shared VS Code trees are never promoted into RCs. **Product source vs harness (D1, 2026-09-05):** a release's product bytes are defined by its source commit; verification scripts and workflows are a harness that may advance afterwards without invalidating artifacts built from that commit. CI-built artifacts (macOS x64, Windows) are signed and packaged from a harness worktree with `RITEMARK_RELEASE_COMMIT=<approved commit>`, which anchors provenance verification to that commit's git objects. Full contract: `docs/development/release-process/BUILD-AND-WORKTREE-HYGIENE.md`.

**Worktree hygiene invariant:** `node ./scripts/worktree-hygiene.mjs --report` is safe to run any time and deletes nothing; `--clean` removes the reviewed worktrees and is **human-authorized only**. Never override `BLOCKED`, and never manually recurse-delete a worktree. A worktree holding build output (a non-empty `dist/` or `VSCode-<target>/`) is always held back — that output is Git-ignored, so a release worktree full of signed, notarized artifacts looks pristine to `git status`. The `worktree-janitor` agent only reports; a local scheduled task mails the report to Jarmo, who decides.
...
│   │   ├── officePreview/       # Word + PowerPoint previews: one provider, host pre-check (Sprint 124/125)
│   │   └── [editors]            # ritemarkEditor, pdfEditorProvider, excelEditorProvider, drawioEditorProvider
│   ├── out/                     # Compiled JS
