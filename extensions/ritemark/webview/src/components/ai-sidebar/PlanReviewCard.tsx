...
export function PlanReviewCard({
  ...
  return (
    <div className="rounded-lg border border-[var(--r-accent-fainter)] overflow-hidden bg-[var(--vscode-input-background)]/80">
      ...
      {/* Keep-planning feedback input — lives in the footer zone, directly
          above the actions it belongs to */}
      {allowFeedback && showRejectInput && (
        <div className="px-2.5 pb-2">
          <input
            ...
            autoFocus
            className="w-full rounded-md border border-[var(--r-hairline)] bg-[var(--vscode-input-background)] px-2.5 py-1.5 text-xs text-[var(--vscode-input-foreground)] outline-none focus:border-[var(--r-accent)] focus:shadow-[0_0_0_4px_var(--r-ring-color,rgba(67,56,202,0.1))]"
          />
        </div>
      )}

      {/* Actions — Approve is the primary indigo CTA. Labels never wrap
          mid-word; buttons share the row 50/50 and stack full-width when the
          sidebar is narrow (flex-1 + wrap). */}
      <div className="flex flex-wrap items-center gap-2 px-2.5 py-2 border-t border-[var(--r-hairline)]">
        <button
          onClick={onApprove}
          className="flex flex-1 basis-[45%] items-center justify-center gap-1.5 whitespace-nowrap rounded-md px-3 py-1.5 text-xs font-semibold text-white bg-[var(--r-accent)] hover:bg-[var(--r-accent-deep)] shadow-[0_4px_6px_-1px_rgba(67,56,202,0.25)] active:scale-[0.98] transition-transform"
        >
        ...
        <button
          ...
          className="flex flex-1 basis-[45%] items-center justify-center gap-1.5 whitespace-nowrap rounded-md border border-[var(--r-hairline)] bg-transparent px-3 py-1.5 text-xs font-medium text-[var(--r-ink-body)] hover:bg-[var(--r-surface-soft)] hover:text-[var(--r-ink-strong)]"
        >
          <Icon name={allowFeedback && showRejectInput ? 'paper-plane-right' : 'x'} size={12} />
