...
Immediately before upload, the tool computes SHA-256 and Content-MD5 from the same file stream and checks the SHA-256 against the approved candidate. It sends that fixed Content-MD5 with PutObject so R2 rejects changed bytes before committing the object. MD5 is a transfer-integrity checksum; SHA-256 remains the approved release identity. A `BadDigest` response is a hard failure, never a reason to retry without checksum validation. This closes the file-change race identified in PR #279. Tests simulate both replacement before reading and modification during streaming; no live production object is used for those tests.
