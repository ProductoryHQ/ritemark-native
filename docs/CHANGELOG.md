...
- **Fast save-and-continue no longer becomes a false disk conflict.** The coordinator matches a later disk snapshot to the exact local content captured at save time, so a user's own delayed save can advance the common base while newer visible typing remains local-only.
