...
### Fixed
- **Claude authentication failures now explain the problem and offer the right recovery in place.** Instead of showing a raw provider error, OAuth chat turns provide **Sign in to Claude** and API-key turns provide **Update API key** through AI Settings. The recovery survives reloads and conversation navigation; shared login completion reaches every joined surface, while Ritemark releases stale Claude sessions and avoids starting a model probe while auth is unavailable. The inline recovery uses Ritemark's neutral card surface, radius, and primary action instead of a bright VS Code validation warning, without repeating the same failure below the card.
