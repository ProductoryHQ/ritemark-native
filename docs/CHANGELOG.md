...
### Fixed
- **Expired Claude sessions now explain the problem and offer recovery in place.** Instead of showing a raw OAuth error, chat says that Claude needs a new sign-in and provides a **Sign in to Claude** button using the same login flow as Settings. Ritemark releases every stale Claude session around re-authentication and avoids starting a model probe while auth is unavailable, reducing the chance that an old process invalidates the fresh credential.
