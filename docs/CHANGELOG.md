...
### Fixed
- **Claude authentication failures now explain the problem and offer the right recovery in place.** Instead of showing a raw provider error, OAuth chat turns provide **Sign in to Claude** and API-key turns provide **Update API key** through AI Settings. Shared login completion reaches every joined surface, while Ritemark releases stale Claude sessions and avoids starting a model probe while auth is unavailable.
