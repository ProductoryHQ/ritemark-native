...
### Added
- **New Claude models appear without an app update (Sprint 127, #343).** Ritemark's public model list gains a new Anthropic model automatically, once a canary has run it on the Claude Code version Ritemark bundles. The app checks the list every 10 minutes. It then declares the model to its bundled Claude Code, so subscription users can pick and run it before that Claude Code knows it natively. Automated models never become the default, and only 1.12.0 and later receive them. A model marked retired in the list is hidden everywhere.
