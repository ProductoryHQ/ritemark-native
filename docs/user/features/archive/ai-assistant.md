> **ARCHIVED — feature removed in v1.7.2.**
> This page documents the **Ritemark Agent** (also called the Legacy Agent): a direct OpenAI chat
> runtime that offered rephrase, find-and-replace, and insert tools, plus document search (RAG).
> Sprint 74 removed this runtime and the RAG subsystem entirely. The shipping AI sidebar now offers
> only **Claude Code** and **Codex** — see [AI Agents](../ai-agents.md). The OpenAI API key is still
> used by **Flows** (LLM Prompt and Image Generation nodes) — see [Set Up AI](../../setup-ai.md).
> This document is kept for historical reference only and does not describe current behavior.
