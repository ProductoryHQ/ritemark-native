...
Save lag is classified by identity, not time. Only successful `onDidSaveTextDocument` completion creates a local-write receipt; the coordinator synchronously hashes the just-written disk bytes rather than the live model, which may already contain newer typing. This also captures format-on-save and code-action output exactly, while a canceled or failed `onWillSave` attempt creates no receipt. If a later disk snapshot matches one of those exact pending receipts while the model has already advanced, the disk snapshot becomes the new common ancestor and the newer model remains local-only. Receipts are consumed through the newest match and retired whenever another authoritative base is established. A disk hash that matches no confirmed local receipt still follows the normal three-way path and can produce a true conflict.
