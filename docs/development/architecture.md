...
**Comment callouts (Sprint 94, #81; assignment rebuilt in Sprint 117).** Editor-only comments live in the editor webview (TipTap `CommentMark` for anchored highlights, an atom `CommentNode` for `///` notes, and a DOM-scanning `MarginCommentRail`); they round-trip through a scoped `marked` tokenizer + Turndown rules and are stripped at the shared export chokepoint (`export/v2/htmlPipeline.ts`). Since Sprint 117 **both** comment kinds carry a stable `data-comment-id`, the standalone Markdown carrier is `<!-- {id:<id>} body -->`, and `webview …/extensions/comment/commentIndex.ts` is the single capture authority behind every surface. Handing a comment to an agent is no longer an editor→sidebar relay — the old `comment:send-to-ai` / `comment:submit` / `comment:task-status` messages and the webview status map are gone. It is now a host-owned task; see [Comment tasks (Sprint 117)](#comment-tasks-sprint-117). Gated by the `comment-callouts` experimental flag (default on). The comment webview.js bundle is cache-busted via `?v=<mtime>` (was silently serving stale bundles across reloads).
...
**Office previews (Sprint 124, #284; Sprint 125, #285).** One provider, `src/officePreview/officePreviewProvider.ts` (`OfficePreviewProvider`, configured per format by `WORD_FORMAT` and `POWERPOINT_FORMAT`: view type, part folder, wording, external apps, flag), serves `ritemark.docxViewer` and `ritemark.pptxViewer` and loads their own bundle, `media/office-preview.js` (`webview/src/office/`, built by `vite build --mode office`), not the shared `webview.js`. Before sending a file the host runs `src/officePreview/officePackageCheck.ts` — size cap (50 MB, the file is not read whole above it), CFB container (password-protected / legacy), ZIP central directory (damaged, entry count, unpacked size and ratio), then every part inflated in the host capped at its declared size + 1 byte, so a part that misstates its size is refused (Sprint 125) — and sends `loadError` with a plain title and detail instead of `load`. In the webview, `components/viewers/docx/prepareDocx.ts` rewrites the package's XML before docx-preview 0.4.1 renders it (`docxXml.ts`: Word's page markers honoured only where present, the marker after a manual break dropped, PAGE / NUMPAGES marked for per-page numbers, empty embedded font faces dropped, unsupported content scanned); `officeFonts.ts` aliases missing Office fonts at 88 %. The Word and PDF viewers share `viewers/ViewerToolbar.tsx` and the pure `viewerLayout.ts`; Word zoom is a transform on a stage inside a sizer (CSS `zoom` gave inconsistent geometry), search uses the CSS Custom Highlight API over `documentSearch.ts` matches, driven from `components/FindBarShell.tsx` — the floating find bar the Markdown editor's `FindBar.tsx` also renders, so the app has one search UX. The PDF viewer (in `webview.js`) searches the same way (#344): `viewers/pdfSearchIndex.ts` indexes every page's text through pdf.js up front, and `viewers/pdfSearchDom.ts` maps a match to the rendered text layer's spans. The highlight colours live in `viewers/documentSearch.css`, which all three viewers import, because a rule in one bundle's stylesheet is absent from the other. Case-folded search helpers are shared with the transcript in `webview/src/utils/textSearch.ts`. The pre-commit hook, `build-prod.sh`, the extension-update file list and the bundle checks all know both bundles.

**PowerPoint (Sprint 125, #285).** `webview/src/components/viewers/PPTXViewer.tsx` draws each slide with `@aiden0z/pptx-renderer` 1.3.0's standalone `renderSlide` into its own list: slides stack in a scroller > sizer > stage transform like the Word viewer, and an IntersectionObserver draws only slides within 1.5 viewports (disposing the rest, which frees their chart canvases). `pptx/loadDeck.ts` parses with the renderer's ZIP limits, rewrites chart XML (`pptx/pptxXml.ts`: `varyColors` off on multi-series bar/line/area, the series name into an empty auto title), reads speaker notes from the notes slides (the renderer does not), aliases fonts installed nowhere (`officeFonts.installFontFallbacks`) and turns chart animation off via ECharts' `registerPreprocessor`. Search counts matches in the model's text per slide (own shapes only, not master/layout) plus the notes (`pptx/deckSearch.ts`), and highlights the words in drawn slides, whose own shapes are the last `slide.nodes.length` children (`pptx/renderedDeck.ts`). Links route through the host (`openLink` → `vscode.env.openExternal`, http(s) only). The bundle writes its third-party notices to `media/office-preview.NOTICES.txt` at build time. The `powerpoint-preview` flag is a kill switch: the editor stays registered and says the preview is off.
...
`src/commentTasks/` is the canonical owner of *"this comment was handed to an agent"*. Before this sprint that fact lived in three places that could disagree — an optimistic editor flash, a module-global map in the editor webview, and a `commentTasks` slice in the AI sidebar store — and none of them survived a reload. Now there is one host ledger and everything else is a projection of it.

| Module | Owns |
|---|---|
| `commentTasks/types.ts` | `CommentTaskRecordV1`, its codecs, the per-document `CommentTaskProjectionV1`, and the lifecycle transition table |
| `commentTasks/protocol.ts` | The exact-field request/result/event union, `decodeCommentTaskRequest`, `commentTaskError`; the only place webview JSON becomes a task request |
| `commentTasks/CommentTaskStore.ts` | Durable records, index, quarantine, per-document query, transitions, retry/cancel, rename rewrite, retention |
| `commentTasks/CommentTaskController.ts` | Task identity and lifecycle. No `vscode` import — every dependency is injected, so acceptance and lifecycle run in Node tests |
| `commentTasks/commentTaskPrompt.ts` | The one prompt builder for one or many comments (the webview's `buildAgentTaskPrompt` and the rail's prose prompt are both deleted) |

Records live under `<globalStorage>/comment-tasks/v1/` with `records/`, `quarantine/`, and `index.json`, reusing the `ConversationStore` file pattern exactly: injected filesystem, one serialized mutation tail, `tmp-<id>`-then-rename writes, quarantine instead of deletion for anything unreadable, and a rebuildable index. A record holds the frozen task inputs and the current lifecycle — never transcript content, tool payloads, approval capabilities, or attachments. Terminal records expire after 30 days, and the store keeps at most 500 (oldest terminal first).

Composition: `extension.ts` constructs the store and controller once at activation beside `ConversationController`, exports both, and calls `recoverUnfinished()` so work abandoned by a crash or a quit becomes an honest `interrupted` instead of a task that looks queued forever. `RitemarkEditorProvider` indexes editor webviews by canonical document URI, forwards every `comment-task/*` message to the controller with a URI + `scopeId` document identity, republishes the ledger on editor `ready`, and follows `workspace.onDidRenameFiles`. `UnifiedViewProvider` owns the sidebar half: it remembers which conversation is open, resolves the destination (revealing the sidebar and waiting up to 5 s for its first report), gates on shared runtime availability, waits for a real enqueue verdict (10 s), reveals a bound conversation on demand, cancels a task's turn, and feeds turn-started / attention / terminal events back into the controller.

```text
editor webview                    extension host                         AI sidebar webview
──────────────                    ──────────────                         ──────────────────
comment-task/destination-preview →  open conversation, title
                                 ←  caption only, no picker
[Send]
comment-task/accept  ──────────→   validate ids in the TextDocument
  requestId, surface, alias,        canonical URI + scopeId
  comments[]                        runtime availability (shared policy)
                                    destination = the open conversation
                                    persist `accepting`
                                                       comment-task/enqueue ───→ queue of THAT
                                                                                 conversation
                                                       ←── comment-task/enqueue-result
                                                            queued | full | no-conversation
                                    persist `queued`
                 ←── comment-task/result { ok, taskId, destination }
                                                                          agent-execute
                                                                          (taskId, host turn id,
                                                                           source path)
                                    conversation turn events
                                    → transition by (taskId, generation)
                                                     × (conversationId, turnId)
                 ←── comment-task/projection (full snapshot, one document)
```

`comment-task/result` correlates by `requestId`; `comment-task/projection` is a full snapshot for one document, posted only to that document's editor webviews. Two further messages complete the loop: the sidebar posts `comment-task/dequeued` when a user removes a queued item (the host records `cancelled`), and the editor posts `comment:recover` — deliberately outside the `comment-task/` prefix so the protocol decoder never sees it — which the editor provider maps to `ritemark.claudeLogin` / `ritemark.codexLogin` / `ritemark.aiSettings`.

**Invariants.**

- **The host owns identity and lifecycle.** Webviews capture intent and render a projection. They mint no task IDs, keep no status map, and never decide a destination.
- **A transition follows the table in `types.ts`,** per binding generation; anything else is rejected and logged. A retry bumps `bindingGeneration`, mints a new turn ID, and appends the previous outcome to `history`, so a late callback from the old attempt cannot move the new one.
- **Every event names its binding.** Lifecycle is derived from the conversation record for the exact `(conversationId, conversationTurnId)`. A terminal event can no longer finish every running task in a conversation (audit F22), and events for another turn, another generation, or an unknown task are ignored.
- **The destination is the conversation open in the AI sidebar** at the moment of sending, a fresh empty one included (D5, decided by Jarmo 2026-09-14). No picker, no confirmation dialog; the Send surface shows the title as a caption, and the task keeps that canonical `conversationId` even if the user then switches conversations.
- **Source binding is canonical URI + project `scopeId`,** never the display-only relative path. Only `file:` documents can be dispatched; `untitled:` is rejected with a "save the document first" recovery. In-app renames follow; external moves are not guessed at.
- **No success before acceptance.** The editor reports `queued` only after the record is durable *and* the sidebar has reported a real enqueue outcome. A full queue answers `queue-full` and leaves no record behind.
- **The agent's answer is a projection.** The bounded completion summary lives in the record and the comment bubble; the user's note, the anchored text, the Markdown carriers, and exports are untouched.
- **Comment identity is stable for both kinds.** Anchored marks and standalone notes both carry `data-comment-id`; legacy comments are upgraded in one undoable transaction at dispatch, and duplicate IDs are re-minted.
- **One availability derivation.** `src/runtime/availability.ts` holds `deriveRuntimeAvailabilities`; the host imports it for the acceptance gate and the sidebar re-exports it, so a signed-out runtime says the same thing in both places.
- **`comment-callouts` stays the only flag** and stays default-on. Flag-off answers `feature-disabled` and sends no projection; comment tasks additionally require the durable conversation store (`rolloutMode !== 'legacy'`), because without host-canonical turns there is no turn identity to bind to.

**Deleted by this sprint** (do not reintroduce): `comment:send-to-ai`, `comment:submit`, `comment:task-status` and `broadcastCommentTaskStatus`, the editor's module-global comment-status map, the sidebar store's `commentTasks` slice with `setCommentTaskStatus` / `clearCommentTask` / `finalizeCommentTasks`, the sidebar's silent destination picking, the webview prompt builder, the rail's "Sent" flash, and the menu's "Queued N tasks" banner.

**Ergonomics (R10).** Two reusable webview primitives landed with the same sprint: `components/comment/ResizableComposer.tsx` (native `resize: vertical`, minimum two rows, maximum `min(8 rows, 40vh)`, footer outside the scrolling area) and `components/comment/AgentMentionPicker.tsx` (a `role="listbox"` opened by `@` at a word boundary, offering exactly `COMMENT_AGENT_ALIASES` with labels from the shared `ALIAS_LABEL`). `components/comment/commentTaskCopy.ts` is the single pure state→text mapping; every state has a glyph and a sentence, so nothing is signalled by colour alone. v1.12.0 Sprint 122 reuses the composer primitive for the agent composer rather than writing a second one.

#### TO BE — deliberate gaps carried out of Sprint 117

| Gap | Where it bites |
|---|---|
| `comment-task/cancel` is decoded and handled, but no webview surface sends it | The protocol branch is unreached; `design.md` gives no state a Cancel action (Stop in the sidebar produces `cancelled`). Confirm with Jarmo whether the editor should expose one. |
| D3's bounded 1 s re-check before `document-not-synced` | The controller does a single containment check, so a Send immediately after an edit can answer `document-not-synced`. It is retryable, so the user is told the truth, but the bounded wait is absent. |
| D6's persistent ready-queue for enqueues | A sidebar that hydrates later than the 10 s window fails the task as `destination-not-found` instead of delivering late. |
| A deleted destination conversation | `interrupted(conversation-deleted)` exists in the record, the projection, and the comment copy, but nothing detects the deletion and transitions the tasks bound to it. |
| `runtime.approvalMode` frozen to `auto` | The per-conversation Ask/Auto toggle is webview state the host cannot read. Honouring it requires the sidebar to report autonomy — a contract change, so it goes back to Jarmo. |
...
**Composer.** Until the person sizes it, `ChatInput` fits its text up to Sprint 117's `composerBounds` ceiling (8 lines or 40vh). `ComposerResizeHandle` on the card's top edge is an ARIA window splitter: drag or arrow keys set an exact height (arithmetic in the pure `composerResize.ts`), double-click returns to fitting the text, and the chosen height is kept per surface for the session. Every height is also capped by the room left in the sidebar column, less a strip of conversation, so the controls row can never be pushed out of view.
...
### Bundled runtime baseline (Sprint 111, RC-corrected 2026-09-03; Windows sandbox helpers added 2026-09-07)

The v1.10.0 host↔binary baseline is exact and reproducible: Codex `0.153.0`, Claude Code `2.1.239` paired with Agent SDK `0.3.239`, and OpenCode `1.18.21` paired with ACP SDK `1.4.0`. `extensions/ritemark/binaries/agents/manifest.json` remains the single binary supply-chain contract with one row per required component × the targets that component actually supports. Most components cover all three targets (`darwin-arm64`, `darwin-x64`, `win32-x64`); a component may legitimately be platform-scoped when the vendor ships no build for a target. Codex has four mandatory, version-matched components. Two are cross-platform: `app-server` and the sibling `code-mode-host` that serves Code Mode file tools. Two are **win32-x64 only**: `windows-sandbox-setup` and `command-runner`, which Codex spawns on its Windows sandbox path. They have no darwin build by construction — on macOS the sandbox is the OS seatbelt rather than a spawned helper process. Omitting either one leaves every Codex file/exec tool failing at runtime with `orchestrator_helper_launch_failed: ... program not found`, which is exactly the defect shipped in the v1.10.0 Windows installer and fixed in v1.10.1. Claude and OpenCode each have one `runtime` component. Codex retains direct GitHub release assets; Claude and OpenCode retain official npm optional-package artifacts. This changes no `AgentRuntime` interface and adds no runtime kind.

`scripts/validate-agent-runtime-manifest.mjs` is the hard pre-fetch/build gate. Schema 2 requires fourteen component rows and rejects an incomplete component/platform matrix — per-component target sets are declared in the validator's `COMPONENT_TARGETS` map, so a platform-scoped component must match its declared targets exactly and a stray darwin row is rejected — duplicate or noncanonical per-target install names, a snapshot outside the approved pins, malformed checksums/source URLs, stale vendor identity, package-lock drift, or a missing/mismatched Claude optional package. It runs from fetch, runtime verification, and the repository QA gate. The canonical install name is part of the runtime contract: Codex discovers `codex-code-mode-host[.exe]` by its exact sibling filename. The manifest fetcher verifies archive SHA-256 before extraction, validates the recorded path/architecture, and runs each component's supported native smoke (`--version` for primary runtimes, `--help` for the code-mode host). The two Windows sandbox helpers are IPC endpoints rather than CLIs — the command runner wants a pipe handle and the sandbox setup wants a base64 payload, so both exit non-zero on `--version` and `--help`. Their rows carry no `validationArgs`, the fetcher skips the smoke test when that field is empty, and the validator **rejects** `validationArgs` on them rather than requiring it; presence beside `codex-app-server` is the check. Build-output validation and the Windows installer preflight both enumerate the manifest, while macOS signing re-signs every executable component under the agent directory. `.github/workflows/agent-runtime-matrix.yml` reruns the exact SDK compile, native fetch, and component smoke on Intel macOS and Windows x64 when supply-chain inputs change. Full installer/signing and a real packaged Codex file-tool canary remain release gates; a macOS cross-fetch alone proves bytes/layout, not native behavior.
...
Vite --mode office → media/office-preview.js   Word + PowerPoint previews: ~2.5 MB IIFE  [Sprint 124, 125]
                        + office-preview.NOTICES.txt (third-party notices)  [Sprint 125]
...
*Amended by Sprint 127 (next section). The live list no longer wins outright: the static layers merge per row, and feed rows can be declared to Claude Code. The layers and the public API below still hold.*
...
|---|---|---|
| Does the model exist? | Anthropic | `GET /v1/models` with the credential the runtime uses: API-key sign-in only (R2). For subscription users it is the runtime's own list plus the declared feed rows. |
| Can the runtime run it? | The runtime | Claude Code's `supportedModels()`, extended by **declarations**. Feed rows with `claudeCode.inject` go to the SDK as `settings.modelPicker` (the flag-settings layer, honored with `settingSources: []`), and the output budget goes in as `CLAUDE_CODE_MAX_OUTPUT_TOKENS`. |
| How is it presented? | Ritemark | A per-row merge of the bundled catalog with the remote/cache feed. |

**Static merge** (`resolver.ts` `mergeStatic()`):
- A row in both documents takes the fresher document's row.
...
**Declarations** (`runtimeDeclarations.ts`, pure):
- Discovery declares every declared row, so the probe lists them.
- A session declares only the model it runs, and the session is rebuilt when that declaration changes (`ClaudeCodeRuntime`).
- `behavesAs` is honored only on curated rows.
- A declaration is dropped once the runtime knows the id natively (compared modulo `[1m]`).
- Retired or malformed ids are never declared. The CLI accepts any id verbatim, so the client validates ids against `CLAUDE_MODEL_ID_PATTERN`.
- With the `remote-model-catalog` flag off, there is no fetch, no injection and no declaration.
...
- **Model IDs and catalog resolution have separate authorities** — canonical IDs and stable defaults live in `src/ai/modelConfig.ts`; `src/ai/modelCatalog/` projects them into provider/runtime lists and resolves availability through live probe → eligible remote → eligible cache → bundled floor. A remote/cache snapshot older than the bundled `updatedAt` cannot hide a newer app baseline. Never hardcode model IDs in runtimes, views, or Flow executors. *Sprint 127:* the static layers merge per row, not first-wins. Existence (Anthropic), runnability (the runtime, extended by feed declarations) and presentation (Ritemark) are separate authorities. Automated feed rows never become defaults.
