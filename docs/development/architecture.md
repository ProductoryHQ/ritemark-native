...
Save lag is classified by identity, not time. Immediately before VS Code saves a tracked document, the coordinator records that model's normalized logical hash; successful save completion records the post-participant snapshot as well, covering format-on-save and code-action edits. If a later disk snapshot matches one of those exact pending receipts while the model has already advanced, the disk snapshot becomes the new common ancestor and the newer model remains local-only. Receipts are consumed through the newest match and retired whenever another authoritative base is established. A disk hash that matches no local receipt still follows the normal three-way path and can produce a true conflict.
