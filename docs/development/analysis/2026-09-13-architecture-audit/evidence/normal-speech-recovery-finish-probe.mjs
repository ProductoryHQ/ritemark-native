...
const save=()=>writeEvidence('normal-speech-recovery-finish-probe.json',report);
...
try{
 ...
 const pidA=prior.pidA,pending={id:prior.pendingJobId},first=await driver('/snapshot',undefined,pidA);report.pidA=pidA;report.pendingJobId=pending.id;
 const current=await inspect(pidA);assert.equal(current.jobs.find(j=>j.id===pending.id)?.state,'transcribing');assert.ok(current.heldEngines.length);stage('resume-existing-live-job-after-selector-error',current);
