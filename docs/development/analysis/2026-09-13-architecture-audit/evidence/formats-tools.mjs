...
export const session=JSON.parse(fs.readFileSync(path.join(evidence,'formats-session.json'),'utf8'));
export const writeEvidence=(name,value)=>fs.writeFileSync(path.join(evidence,name),JSON.stringify(value,null,2)+'\n');
export async function targets(){return (await fetch('http://127.0.0.1:9239/json/list',{signal:AbortSignal.timeout(1500)})).json();}
...
export async function findWebview(predicate,timeout=15000){
 ...
 throw new Error('Matching formats-mode webview not found: '+predicate);
}
export async function launch(){
 if(!session.runDir.startsWith('/private/tmp/ritemark-audit-formats-'))throw new Error('Wrong run directory');
 ...
 const env={...process.env,RITEMARK_AUDIT_FORMATS_RUN_DIR:session.runDir};
 ...
 const fd=fs.openSync(path.join(session.runDir,'app.log'),'a'),at=Date.now();
 const child=spawn(session.args[0],session.args.slice(1),{env,stdio:['ignore',fd,fd],detached:true});fs.closeSync(fd);child.unref();
 (session.launches??=[]).push({pid:child.pid,at});Object.assign(session,{pid:child.pid,launchedAt:at,preparedOnly:false});writeEvidence('formats-session.json',session);
