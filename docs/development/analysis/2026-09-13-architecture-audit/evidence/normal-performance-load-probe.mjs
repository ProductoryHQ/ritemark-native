...
// Flattened CDP sessions let us pause a newly created webview before its scripts
// run. Only this disposable profile is addressed, via its private CDP port.
const file='normal-performance-load-probe.json';
const report={method:session.method,at:Date.now(),pid:session.pid,fixture:session.fixture,
  ...
  instrumentation:'Auto-attach/pause new iframe, CPU throttle, Performance metrics, 1 ms CPU sampling and precise block coverage before resume. Main-thread long tasks recorded by audit observer. All trials instrumented; timings are not uninstrumented production benchmarks.',trials:[],errors:[]};
...
async function setupAttached(p){
 ...
 try{
  await send('Target.setAutoAttach',{autoAttach:true,waitForDebuggerOnStart:true,flatten:true,filter},p.sessionId);
...
try{
 ...
 await send('Target.setAutoAttach',{autoAttach:true,waitForDebuggerOnStart:true,flatten:true,filter});
 await sleep(700);
 ...
 for(const [index,rate] of [1,4,4,1,1,4].entries()){
  ...
  assert.ok(found.item.waitingForDebugger,'New target must have paused before scripts');
  ...
  trial.profileFile=`normal-performance-trial-${trial.index}.cpuprofile`;
  trial.coverageFile=`normal-performance-trial-${trial.index}-coverage.json`;
