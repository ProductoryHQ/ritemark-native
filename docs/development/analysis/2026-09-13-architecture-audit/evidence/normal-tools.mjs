...
export const session=JSON.parse(fs.readFileSync(path.join(evidence,'normal-session.json'),'utf8'));
export const writeEvidence=(name,value)=>fs.writeFileSync(path.join(evidence,name),JSON.stringify(value,null,2)+'\n');
export async function targets(){return (await fetch('http://127.0.0.1:9238/json/list',{signal:AbortSignal.timeout(1500)})).json();}
...
export async function launch(){
 ...
 (session.launches??=[]).push({pid:child.pid,at});Object.assign(session,{pid:child.pid,launchedAt:at,preparedOnly:false});writeEvidence('normal-session.json',session);
