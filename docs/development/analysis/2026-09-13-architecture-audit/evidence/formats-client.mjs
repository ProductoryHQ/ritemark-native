...
import {driver,targets,connectCdp,findWebview,launch,evidence,writeEvidence} from './formats-tools.mjs';
console.log('FORMATS_AUDIT_CLIENT_READY');
...
for await(const line of input){
 ...
 try{
  ...
  if(q.action==='launch')result=await launch();
   ...
   else{if(!/^formats-[a-z0-9-]+\.png$/.test(q.file))throw new Error('Invalid filename');const r=await c.send('Page.captureScreenshot',{format:'png'});fs.writeFileSync(path.join(evidence,q.file),Buffer.from(r.data,'base64'));result={file:q.file};}
  }else if(q.action==='probe'){if(!/^formats-[a-z0-9-]+-probe\.mjs$/.test(q.file))throw new Error('Invalid probe filename');await import('./'+q.file+'?run='+Date.now());result={probe:q.file,finished:true};}
  else if(q.action==='exit'){input.close();break;}
  else throw new Error('Unknown action');
  if(q.evidence){if(!/^formats-[a-z0-9-]+\.json$/.test(q.evidence))throw new Error('Invalid evidence filename');writeEvidence(q.evidence,result);}
