...
exports.activate=context=>{
 ...
 if(roots.length!==0||vscode.workspace.workspaceFile)throw new Error('Only no-folder store audit windows are allowed');
 ...
 const storeAudit=require('./store-audit-api.cjs')(context,runDir,log);
 const server=http.createServer(async(req,res)=>{
  ...
  try{
   ...
   if(req.url==='/store-audit')return reply(200,await storeAudit(q));
   if(req.url==='/open'){
