...
const file='normal-performance-profile-load-probe.json';
const report={method:session.method,at:Date.now(),pid:session.pid,fixture:session.fixture,
  ...
  instrumentation:'Auto-attach without pausing new iframe, focus emulation avoids macOS occlusion; CPU throttle, Performance metrics, 1 ms CPU sampling and precise block coverage with metric start checked against bundle request; profiler start checked separately and may follow download/compilation. Main-thread long tasks collected by buffered observer. All trials instrumented; timings are not uninstrumented production benchmarks.',trials:[],errors:[]};
...
async function setupAttached(p){
 ...
 try{
  ...
  if(activeTrial&&item.type==='iframe'){
   ...
   await send('Emulation.setFocusEmulationEnabled',{enabled:true},p.sessionId);
   ...
   item.metricsStartedAt=Date.now();
...
try{
 ...
 for(const [index,rate] of [1,4].entries()){
  ...
  while(Date.now()-trial.startedAt<25000&&!found){
   for(const item of trial.attached.filter(i=>i.instrumented&&!i.detachedAt)){
    for(const ctx of contexts.get(item.sessionId)?.values()??[]){
     ...
     try{const value=await evaluate(item.sessionId,ctx.id,`(()=>{const e=document.querySelector('.tiptap[contenteditable="true"]');return e&&e.textContent.length>10&&window.frameElement?.id==='active-frame'&&document.visibilityState==='visible'?{visibility:document.visibilityState,frameId:window.frameElement?.id,text:e.innerText,html:e.innerHTML,now:performance.now(),timeOrigin:performance.timeOrigin,editorType:document.querySelector('#root')?.dataset.editorType??null}:null})()`);
  ...
  trial.metricsBeforeBundleRequest=found.item.metricsStartedAt<trial.scriptRequestEpoch;
  trial.profilerBeforeBundleRequest=found.item.profileStartedAt<trial.scriptRequestEpoch;
  ...
  trial.profileFile=`normal-performance-profile-trial-${trial.index}.cpuprofile`;
  trial.coverageFile=`normal-performance-profile-trial-${trial.index}-coverage.json`;
 ...
 assert.equal(new Set(report.trials.map(t=>t.targetId)).size,2);
 ...
 activeTrial=null;
 for(const item of attached.values())if(!item.detachedAt){try{await send('Emulation.setCPUThrottlingRate',{rate:1},item.sessionId);await send('Emulation.setFocusEmulationEnabled',{enabled:false},item.sessionId);await send('Runtime.runIfWaitingForDebugger',{},item.sessionId);}catch{}}
