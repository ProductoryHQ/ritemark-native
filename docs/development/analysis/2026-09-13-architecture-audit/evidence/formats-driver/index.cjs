...
exports.activate=context=>{
 const runDir=process.env.RITEMARK_AUDIT_FORMATS_RUN_DIR;
 if(!runDir?.startsWith('/private/tmp/ritemark-audit-formats-'))throw new Error('Synthetic normal-mode run required');
 ...
 const allowedFiles=new Set(['note.md','diagram.drawio.svg','sample.pdf','sample.docx']);
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  try{
   ...
   if(req.url==='/open'){
    ...
    await vscode.commands.executeCommand('vscode.openWith',vscode.Uri.file(path.join(workspace,q.file)),q.file.endsWith('.drawio.svg')?'ritemark.drawioEditor':q.file.endsWith('.pdf')?'ritemark.pdfViewer':q.file.endsWith('.docx')?'ritemark.docxViewer':'ritemark.editor',{preview:false});
