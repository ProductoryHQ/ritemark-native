...
export const session=JSON.parse(fs.readFileSync(path.join(evidence,'normal-store-session.json'),'utf8'));
export const writeEvidence=(name,value)=>fs.writeFileSync(path.join(evidence,name),JSON.stringify(value,null,2)+'\n');
export async function targets(){return (await fetch('http://127.0.0.1:9240/json/list',{signal:AbortSignal.timeout(1500)})).json();}
...
export async function launch(){
 ...
 const fd=fs.openSync(path.join(session.runDir,'store-app.log'),'a'),at=Date.now();
 const child=spawn(session.args[0],session.args.slice(1),{env,stdio:['ignore',fd,fd],detached:true});fs.closeSync(fd);child.unref();
 (session.launches??=[]).push({pid:child.pid,at});Object.assign(session,{pid:child.pid,launchedAt:at,preparedOnly:false});writeEvidence('normal-store-session.json',session);
