...
report.before=all.filter(p=>own.has(p.pid));writeEvidence('formats-security-cleanup-probe.json',report);
...
if(remaining.length){writeEvidence('formats-security-cleanup-probe.json',report);throw new Error('Known audit processes remain; inspect before any further action');}
...
for(const n of eventFiles)fs.copyFileSync(path.join(session.runDir,'events',n),path.join(evidence,'formats-security-driver-events-'+n));
report.eventFiles=eventFiles.map(n=>'formats-security-driver-events-'+n);report.completed=true;writeEvidence('formats-security-cleanup-probe.json',report);console.log(JSON.stringify({completed:true,stoppedProcesses:report.before.length,cdpPortClosed:report.cdpPortClosed,eventFiles:report.eventFiles}));
