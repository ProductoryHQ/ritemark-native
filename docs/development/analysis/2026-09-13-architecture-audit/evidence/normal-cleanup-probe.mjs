...
import {session,driver,targets,connectCdp,processList,writeEvidence,evidence} from './normal-tools.mjs';
const report={method:'Verify the exact synthetic normal-mode app PID/profile and descendants, close via CDP and verify every captured process plus its command has disappeared. Preserve only synthetic driver events; endpoint bearer tokens are not copied.',at:Date.now(),pid:session.pid,hosts:await driver('/snapshots')};
...
report.before=all.filter(p=>own.has(p.pid));writeEvidence('normal-cleanup-probe.json',report);
...
if(remaining.length){writeEvidence('normal-cleanup-probe.json',report);throw new Error('Known audit processes remain; inspect before any further action');}
...
for(const n of eventFiles)fs.copyFileSync(path.join(session.runDir,'events',n),path.join(evidence,'normal-driver-events-'+n));
report.eventFiles=eventFiles.map(n=>'normal-driver-events-'+n);report.completed=true;writeEvidence('normal-cleanup-probe.json',report);console.log(JSON.stringify({completed:true,stoppedProcesses:report.before.length,cdpPortClosed:report.cdpPortClosed,eventFiles:report.eventFiles}));
