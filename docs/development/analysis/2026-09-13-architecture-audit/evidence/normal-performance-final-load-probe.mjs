...
const file='normal-performance-final-load-probe.json';
const report={method:session.method,at:Date.now(),pid:session.pid,fixture:session.fixture,
  ...
  instrumentation:'Auto-attach without pausing, focus emulation to avoid macOS window occlusion, renderer-only CPU throttle, Performance metrics and buffered long-task observer; no CPU profiler or coverage. Every metric baseline precedes bundle request. Fresh active visible inner frame per trial; app/OS/worker caches warm.',trials:[],errors:[]};
...
try{
 ...
 for(const [index,rate] of [1,4,4,1,1,4].entries()){
  ...
  assert.ok(trial.metricsBeforeBundleRequest,'Performance metric baseline must precede bundle request');
