...
- [ ] At least three browser tabs can open without state bleed or layout corruption.
...
- [ ] Decide storage scope default: workspace vs ephemeral.
- [x] Decide `.html` default opener after native browser smoke.
- [ ] Hide/rename any browser UI actions that are not appropriate for Ritemark.
- [ ] Document limitations that remain for downloads, permissions, popups, and certificates.
...
- [ ] Hand off to user for smoke only after all required tests pass.
