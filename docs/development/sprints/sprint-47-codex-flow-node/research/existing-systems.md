...
| `codexAppServer.ts` | JSON-RPC client (EventEmitter). Key methods: `ensureInitialized()`, `threadStart({ cwd })`, `turnStart(threadId, message, model)`, `sendApprovalResponse(requestId, decision)`, `on('server-request', ...)`, `on(method, ...)` |
