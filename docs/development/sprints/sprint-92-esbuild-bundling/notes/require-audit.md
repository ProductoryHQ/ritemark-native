...
1. **esbuild `external` list** (for the `out/extension.js` entry): `['vscode', 'fsevents', ...node-builtins]`. The two ESM agent SDKs need no explicit `external` entry (esbuild can't see them) but MUST NOT be deleted from `node_modules`.
2. **Open Question RESOLVED — node_modules does NOT fully collapse.** esbuild inlines the pure-JS statically/visibly-imported deps (openai, docx, xlsx, marked, gray-matter, papaparse, pdfkit, turndown(+gfm), zod, cron-parser, node-html-parser, posthog-node, ajv, ajv-formats), but the shipped extension must retain `node_modules/@anthropic-ai/claude-agent-sdk`, `node_modules/@agentclientprotocol/sdk` (+ their transitive deps) and `fsevents`. So the packaging shrinks massively but not to zero.
