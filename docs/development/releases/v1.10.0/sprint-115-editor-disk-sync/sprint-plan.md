...
| Local autosave lag is misclassified as external change | High | Explicit base/model/disk state; exact successful-save disk receipts; no time-window or unconfirmed pre-save guessing; continuous-typing and rapid save-and-continue tests. |
...
| 2026-08-31 | Match delayed disk snapshots to exact confirmed local save receipts | A user can type again before the previous save becomes observable on disk. Successful `onDidSaveTextDocument` completion synchronously hashes the bytes just written, covering ordinary, format-on-save, and code-action-modified saves without trusting the potentially newer live model. Canceled/failed attempts create no receipt; an unknown disk hash still takes the normal three-way conflict path. |
