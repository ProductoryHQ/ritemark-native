...
| 2026-08-31 | Match delayed disk snapshots to exact local save receipts | A user can type again before the previous save becomes observable on disk. Capturing logical content at both `onWillSaveTextDocument` and successful `onDidSaveTextDocument` covers ordinary and save-participant-modified saves; a matching disk snapshot advances the common base while preserving newer model text as local-only, while an unknown disk hash still takes the normal three-way conflict path. |
