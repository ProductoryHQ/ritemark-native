...
**Status:** In development — Sprints 109–112 are merged; Sprint 112 landed through [PR #215](https://github.com/ProductoryHQ/ritemark-native/pull/215) at `b190aaa` with review polish through [PR #216](https://github.com/ProductoryHQ/ritemark-native/pull/216) at `fb0d3a3`; Sprint 113 is rebased on that merged baseline with its combined webview bundle and independent full QA complete, while its authenticated manual UI/accessibility matrix remains open; native Windows remains at the v1.10 candidate gate<br>
**Target:** v1.10.0<br>
...
6. **Create useful Transcribe Insights artifacts** — choose Auto, Estonian, or English, retain full Unicode speaker names, and create a separate no-overwrite Markdown document without changing the transcript.
...
- Insights language is explicit provenance: Auto resolves Estonian/English from the transcript and falls back to English, while legacy Insights remain identified as English.
...
- Shared Auto/Estonian/English Insights language selection, stored selected/resolved provenance, and deterministic language instructions that preserve names, quotes, and timestamps.
...
| [Sprint 112 — Composer Thinking Effort](./sprint-112-composer-thinking-effort/sprint-plan.md) | [#206](https://github.com/ProductoryHQ/ritemark-native/issues/206) | Closed | `codex/sprint-112-composer-thinking-effort` | [#215](https://github.com/ProductoryHQ/ritemark-native/pull/215) | Merged as `b190aaa` | Exact-pin capability probes, all-level adapter matrix, conversation regressions, live RunDev matrix, code review, and official QA pass | User docs, release notes, checklist, and one release screenshot complete | Complete 2026-08-24 |
| [Sprint 113 — Transcribe Insights Output](./sprint-113-transcribe-insights-output/sprint-plan.md) | [#208](https://github.com/ProductoryHQ/ritemark-native/issues/208) | In sprint | `codex/sprint-113-transcribe-insights-output` | [#217](https://github.com/ProductoryHQ/ritemark-native/pull/217) (draft) | Rebased onto Sprint 112 plus merged review polish at `fb0d3a3`; combined webview bundle regenerated at implementation commit `8752982`; merge pending | Focused tests, extension/webview typechecks/build, repository QA, independent full QA, and final responsive UI rerun pass; authenticated model generation and final Create mutation pending | Changelog, release notes, checklist, user docs, and architecture docs drafted | QA handoff — authenticated mutation and merge open |
...
- [ ] Transcribe Insights resolves Auto/Estonian/English honestly, preserves full Unicode speaker names and source quotes, and creates collision-safe separate Markdown snapshots without mutating the transcript.
...
7. **Sprint 113:** in the explicitly approved parallel worktree, implement the approved Auto/Estonian/English, provenance, full-speaker-name, and separate exclusive-create Insights contract; after Sprint 112 merges, rebase and regenerate the shared webview bundle before QA, PR, and merge.
...
11. **Gate 1 and tag:** notarize/staple the approved arm64 DMG, verify notarization, signature, and mounted-app hard checks, then clear technical Gate 1. Make the development repository private before pushing the tag. Only after Gate 1 may the already-pushed version commit be tagged/pushed to start x64/Windows CI.
12. **x64/Windows candidate:** download/sign/package the x64 DMG without notarizing it and prepare the Windows installer; after x64/Windows CI completes, restore the development repository to public. Jarmo tests both platform candidates and gives final Gate 2 approval. Wait at least 60 minutes from the x64 DMG build with no new bug; any x64 rebuild resets its clock and Gate 2 approval.
...
- Sprint 113 had no product dependency on Sprint 112 and was approved to implement in parallel. Its branch is now rebased on merged Sprint 112 plus review polish at `fb0d3a3`, and `media/webview.js` was regenerated from the combined source at `8752982`; the remaining gate is the authenticated manual UI/accessibility matrix.
...
| Language choice corrupts quotes, timestamps, or full Unicode speaker names | High | Shared allowlist/provenance, explicit prompt constraints, normalized full-name path, and Estonian/English fixture coverage | Automated evidence passes — Sprint 113; manual UI pending |
