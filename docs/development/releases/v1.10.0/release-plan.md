...
**Status:** Release candidate rebuild in progress from exact clean `main`. PR #237 fixed the storage-full Agent Chat bootstrap failure and PR #238 restored the Ritemark-only Help menu; both are merged. The signed/unnotarized arm64 DMG built from `b811a18` remains invalid. Gate 1 restarts only with a fresh candidate whose source provenance, mounted contents, signature, architecture, version, timestamp, hash, and packaged regressions are reverified; notarization, the 60-minute clock, and Sprint 114 immutable hosting/Store/SAC-On/exact-hash work remain open.<br>
**Target:** v1.10.0<br>
