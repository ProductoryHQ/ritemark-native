...
| Claude OAuth expiry exposes raw copy and stale warm sessions survive re-login | High | Stable auth failure category, inline shared sign-in flow, app-global stale-session disposal, and no unauthenticated discovery probe | Mitigated on `codex/fix-claude-auth-recovery`; focused automation and RunDev visual evidence required before merge |
