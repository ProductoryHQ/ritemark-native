...
**Status:** Release candidate blocked on `codex/fix-chat-bootstrap-quota` / [PR #237](https://github.com/ProductoryHQ/ritemark-native/pull/237). The signed/unnotarized arm64 DMG built from `b811a18` is invalidated: a real upgraded profile filled webview storage during the obsolete global→workspace conversation copy, and the resulting uncaught `QuotaExceededError` aborted `agent:bootstrap` before catalogs committed. Fresh-profile/two-window evidence did not cover that migration state. Gate 1 restarts only after storage-full packaged-app proof, review, merge, and a fresh clean-`main` build; notarization, the 60-minute clock, and Sprint 114 immutable hosting/Store/SAC-On/exact-hash work remain open.<br>
**Target:** v1.10.0<br>
...
| Legacy conversation storage reaches quota during Agent Chat bootstrap | Critical | Commit catalogs/selection before conversation hydration; make workspace context side-effect-free; select host/legacy authority only after rollout response; inventory host-mode legacy records read-only; never duplicate global records during bootstrap; require quota-full source and packaged-app tests | Reproduced in invalid `b811a18` RC and deterministically in two failing tests; correction active on `codex/fix-chat-bootstrap-quota` / PR #237, Gate 1 blocked |
| VS Code Help contributions reappear after an upstream rebase | Medium | Apply one Ritemark-only fail-closed allowlist at the shared menubar rendering boundary; cover unknown contributions and non-Ritemark behavior; require native-menu visual evidence | Automated policy, compile, clean-patch, and RunDev accessibility gates pass on `codex/fix-ritemark-help-menu`; native screenshot and packaged-RC checks remain |
