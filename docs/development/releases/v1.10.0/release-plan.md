...
| Claude authentication expiry exposes raw copy and stale warm sessions survive recovery | High | Auth-method-aware failure categories, joined login terminal fan-out, inline Settings/API-key actions, app-global stale-session disposal, and no unauthenticated discovery probe | Mitigated on `codex/fix-claude-auth-recovery`; focused automation and RunDev visual evidence required before merge |
