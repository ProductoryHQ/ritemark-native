...
- **Gate 1 / arm64:** packaged Claude picker/mismatch replay; authenticated restart and failure injection; reduced-motion visual verification; residual Insights positive custom-language generation plus negative/accessibility/theme rows; editor-sync receipt-loss, focused/blurred, continuous typing, delayed autosave, burst writes, front-matter/properties/comments/images/CSV integrity, accessibility/theme/zoom, generic-process/formatter/exact-runtime origins, multi-root/standalone, rapid/large-file, lifecycle, previous-epoch, and non-cooperating Keep-local race-injection rows; signed/notarized/stapled DMG install, signature, mount, Gatekeeper, content, and hash verification. Jarmo tests only after the final artifact has passed those technical checks.
- **Gate 2 / x64 + Windows:** signed/notarized/stapled Intel DMG and verified Windows installer testing on the exact tagged artifacts. Any rebuild or post-test artifact mutation resets the corresponding 60-minute stability clock and approval.
...
11. **arm64 candidate and Gate 1:** build/sign the arm64 app and DMG; notarize and staple the DMG; verify notarization, app/DMG signatures, Gatekeeper, mounted-app content, architecture, version, timestamps, and SHA-256; then generate/finalize the test checklist. The 60-minute stability clock starts from the final DMG bytes. Jarmo installs/tests that exact verified artifact, the full 60-minute no-new-bug window elapses, and only then explicitly approves it; any rebuild or mutation resets the clock and approval.
12. **Tag and CI fan-out:** only after arm64 technical verification plus Jarmo's exact-artifact Gate 1 approval, make the development repository private, tag the already-pushed version commit, and push the tag.
13. **x64/Windows candidate, Sprint 114 external matrix, and Gate 2:** manually dispatch the x64 and Windows workflows against that exact tag; download/sign/package/notarize/staple/verify the x64 DMG and verify the Windows installer before human testing. After both workflows complete, restore the development repository to public. The 60-minute x64 stability clock starts from the final DMG bytes. Publish the exact verified Windows bytes at the immutable versioned URL, submit that same SHA-256 to Partner Center, pass preprocessing/certification, Kristiina's clean Windows 11 SAC-On and Store-origin install matrix, and record the Store/direct-download hash match in [#212](https://github.com/ProductoryHQ/ritemark-native/issues/212). Jarmo then tests the exact verified Intel DMG and Windows installer; only after the full 60-minute x64 no-new-bug window elapses does he approve those exact hashes and close #212 to give final Gate 2 approval. Any rebuild or mutation resets the corresponding clock, external evidence, and approval.
14. **Publication without artifact mutation:** only after #212 is closed and final Gate 2 approval is recorded, recheck the approved asset hashes and signatures without rewriting them; regenerate, verify, and publish the canonical update feed together with the matching GitHub release assets. No publication occurs before the complete Sprint 114 external matrix, exact-hash approval, and final verification.
