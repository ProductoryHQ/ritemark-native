...
**Status:** Release candidate rebuild in progress; Gate 1 is open. The earlier `0140ab9` candidate and its approval were invalidated by subsequent release-pipeline and RC regression fixes. The immutable target-copy/source gate is merged, and PR [#245](https://github.com/ProductoryHQ/ritemark-native/pull/245) now normalizes Claude SDK `is_error` authentication failures, repairs affected RC conversation records, and passes full repository QA plus isolated RUNDEV reload/visual validation. Build the next signed, unnotarized arm64 candidate only from a fresh physical checkout of the exact current `origin/main`; Sprint 114 immutable hosting/Store/SAC-On/exact-hash work remains open for Gate 2.<br>
**Target:** v1.10.0<br>
