...
**Status:** Release candidate blocked by a confirmed packaging defect — the signed/unnotarized arm64 candidate from `main` commit `3f01ef5` omitted Codex 0.149.0's required `codex-code-mode-host`, so a real file-tool turn failed even though chat startup and static bundle checks passed. The focused RC correction makes required runtime components manifest-enforced across all targets. After that correction merges, Gate 1 restarts from a fresh clean-`main` build; notarization, the 60-minute clock, and Sprint 114 immutable hosting/Store/SAC-On/exact-hash work remain open.<br>
**Target:** v1.10.0<br>
