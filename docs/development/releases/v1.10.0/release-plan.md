...
**Status:** Release candidate blocked while the focused Agent Chat bootstrap correction on `codex/fix-model-label-window-init` / [PR #236](https://github.com/ProductoryHQ/ritemark-native/pull/236) completes merge. The prior signed/unnotarized arm64 artifact is invalidated: a new window could load a stale Agent Chat bundle and remain at `Model`, while the monolithic host handshake allowed slow runtime/auth/keychain/discovery work to withhold all catalogs. After this correction merges, Gate 1 restarts from a fresh clean-`main` build; notarization, the 60-minute clock, and Sprint 114 immutable hosting/Store/SAC-On/exact-hash work remain open.<br>
**Target:** v1.10.0<br>
