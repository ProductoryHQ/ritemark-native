...
**Status:** Release candidate rebuild required after a deterministic Windows Gate 2 pipeline failure. The exact-`main` arm64 DMG from `0140ab9` passed automated checks, Jarmo's installed-app test, the four-hour hardening window, Apple notarization, stapling, and final Gatekeeper verification. Windows run `33642850280` then proved that its copy step mutated tracked source and that patched-source verification still contained a Windows-sensitive reverse-patch heuristic; the repository was restored to public and obsolete x64 run `33642854001` was cancelled. The immutable target-copy/source-gate correction must merge before Gate 1 restarts from a new exact `main` commit; Sprint 114 immutable hosting/Store/SAC-On/exact-hash work remains open.<br>
**Target:** v1.10.0<br>
