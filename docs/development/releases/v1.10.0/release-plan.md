...
| Claude authentication expiry exposes raw copy, loses recovery after navigation, and leaves stale warm sessions alive | High | Durable auth-method-aware terminal categories, projection restore, joined login terminal fan-out, inline Settings/API-key actions, app-global stale-session disposal, and no unauthenticated discovery probe | Mitigated on `codex/fix-claude-auth-recovery`; focused automation and RunDev visual evidence pass, packaged-RC recheck remains |
