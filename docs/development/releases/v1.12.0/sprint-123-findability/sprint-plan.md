...
- [ ] Searching a long transcript shows the count, moves between matches with the buttons and with Enter / Shift+Enter, and highlights the current match distinctly.
- [ ] A match inside a word the engine was unsure about is found and highlighted, and the unsure-word marking remains.
- [ ] Going to a match pauses Follow playback; "Back to playing line" resumes it; playback and speaker controls keep working while searching.
- [ ] Escape and clearing the field restore the normal transcript; "No matches" is shown when nothing matches.
- [ ] Browser tab labels show the page title without the URL (already true; re-verified on the build).
- [ ] On a fresh profile, a crowded tab row shrinks long names with an ellipsis, and the hover still shows the full name; active tab, close buttons and keyboard switching unchanged.
...
One behaviour choice is mine rather than the issue's: the **Back to playing line**
button also appears after you scroll the transcript with the mouse wheel, not only
after a search, because today that pause has no visible way back either. Say the
word if you would rather keep it to search only.
