...
**Status:** Plan — awaiting Jarmo's approval of the number bound (see Open Decision) and the sprint plan as a whole.<br>
...
- Fix the adjacent-list merge/renumber defect found in Phase 0, so a conversion (real or suppressed) can never change an existing list's `start`.
...
| Adjacent-list merge/renumber fix | The list directly below a new paragraph is never renumbered by a conversion, real or suppressed. |
...
- [ ] Typing `1.`–`999.` at the start of a paragraph still converts it to an ordered list.
- [ ] Typing a number above an existing ordered list never changes that list's `start`, whether or not the new paragraph converts.
...
| Adjacent-list fix changes behaviour for real nested/continued lists | Medium | Test coverage targets only the empty-paragraph-above-a-list case; toolbar/slash-menu list creation is unaffected. |
...
| 2026-09-22 | Proposed bound: convert `1.`–`999.` only, no year-specific logic | Audit recommendation, Option A; pending Jarmo's approval |

## Planning Approval

- [ ] Jarmo approves the number-bound decision (Option A, B, or C).
