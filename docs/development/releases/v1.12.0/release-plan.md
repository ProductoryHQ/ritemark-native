...
| [Sprint 124](./sprint-124-word-preview-fidelity/sprint-plan.md) | Word preview fidelity | Word documents get PDF-like page viewing and measured fidelity improvements | [#284](https://github.com/ProductoryHQ/ritemark-native/issues/284) | none; establishes the Office-preview asset boundary | In review 2026-09-24 |
...
- [ ] Sprint 127 merged; its issue closed or explicitly deferred with evidence; *(Revised by D4.)* `npm run check:anthropic-models` passes on the release source. The candidate bundles Claude Code 2.1.281 (runtime matrix plus `verify-agent-runtimes.sh` on the release Mac). On a release candidate, a subscription account sees and runs Opus 5.5.
...
| Sprint 125 | `sprint-125-powerpoint-preview` | [#350](https://github.com/ProductoryHQ/ritemark-native/pull/350) | #285 | in review; RunDev-validated in a dev build, evidence in the sprint's `qa-evidence.md` | full `npm test` + both typechecks green; `validate-qa.sh` passed | drafted in `docs/releases/v1.12.0/release-notes.md` |
...
| Sprint 127 | `sprint-127-day-zero-models` | [#347](https://github.com/ProductoryHQ/ritemark-native/pull/347); follow-ups [#349](https://github.com/ProductoryHQ/ritemark-native/pull/349), [#352](https://github.com/ProductoryHQ/ritemark-native/pull/352) | #343 | **Merged 2026-09-24** as `b10b0575`; follow-up #349 merged 2026-09-25 as `be201eca`; follow-up #352 in review. D4 scope change 2026-09-24 (Claude Code 2.1.281, pre-release check, publisher withdrawn). Local `sprint-127-day-zero-models`, pushed to `claude/anthropic-models-bundled-cli-gtjld9` | full `npm test` green and `tsc --noEmit` clean after the D4 change (2026-09-24) and each follow-up; `check:anthropic-models` passes; `validate-qa.sh` not yet run from a dev checkout (the cloud checkout has no VS Code submodule); manual QA not run | drafted in `docs/releases/v1.12.0/release-notes.md` |
