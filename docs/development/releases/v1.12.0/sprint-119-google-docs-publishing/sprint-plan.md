...
- **2026-09-21:** Local images **are** in v1. A `data:` URI is capped at 2 KB by the Docs API, so images are published by staging: a temporary copy in the user's own Drive, link-readable for a few seconds, then unshared and deleted. Google keeps its own copy, verified after deletion. A `.ico`, an `.svg` or a missing file is named to the user instead of being dropped. — Jarmo asked whether base64 inline would work; it does not at real sizes, and this is what does.
- **2026-09-20:** The conversion adapter is the **native Docs API**, decided by Claude under Jarmo's delegation ("tee ise ja vasta ise oma küsimustele"). It is the only candidate that keeps a template and refuses a stale write, and it has the best measured structural fidelity. Cost accepted: images are published by staging a temporary copy through the user's own Drive, measured at about 7 seconds for six images. See [integration-decisions.md](./research/integration-decisions.md) § Decision 2 and § Images.
