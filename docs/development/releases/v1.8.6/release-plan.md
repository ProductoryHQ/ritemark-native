...
| 2026-08-05 | **v1.8.6 candidate built — Gate 1 (technical) PASSED.** Version bumped (#187), clean-out prod build from main `f6398d5`+, all 13 patches applied. Verified in the packaged app: Info.plist stamped 1.8.6 (Finder fix), product.json carries configurationDefaults (trust off), fresh bundles (reconcileWithDisk/Plan-only present), patch-013 wiring + writer-voice strings in workbench. Runtime Gate 1 on a fresh profile with a loose .md: opened directly in the Ritemark editor, NO trust dialog, NO Restricted Mode, Home first (v186-candidate-gate1.png). Signed (46 components, agent binaries TeamID JKBSC3ZDT5 verified), DMG via ditto+hdiutil fallback: `dist/Ritemark-1.8.6-darwin-arm64.dmg` (553MB), signed, NOT notarized. DMG timestamp 21:25 — 60-min hardening clock running. Next: Jarmo Gate 2 on the un-notarized DMG; notarize+staple only after approval + hardening; Windows/x64 CI only after Mac gate. | Release execution |
| 2026-08-05 | Sprint 107 (Clean Start) implemented to completion in the main session (the earlier "other session" produced nothing — corrected). R1 shipped as patch 013 via the static-product-import variant (IProductService DI impossible at the positional construction site); live spike PASSED: fresh profile + loose .md → Ritemark editor, no trust modal, no Restricted Mode. R2 shipped per D1 Option A + D2 grandfather; consent toast live-verified (Allow grants + rearms). R3 shipped with pure candidate selection + unit tests. Codex PR-review round applied on top (PRs #176/#178/#182/#183/#184 findings): line-suffix root links, self-content seeding of loaded disk state, one-shot model pin actually forwarded to the SDK, assignment-only mention stripping. Remaining before candidate: win32 spike variants + scenarios walk ride the release-candidate build. | Sprint 107 execution |
| 2026-08-05 | Jarmo's live test pass produced six shipped fixes (PRs #175–#178), three with explicit decisions: (1) **Terminology = Folder** — sidebar view renamed Project→Folder, "Workspace" removed from the UI; (2) **File menu full cut** — New Document (⌘N rebound)+New Table replace New Text File/New File…, workspace items ×4 + profiles + Share + Revert File hidden, Selection menu hidden entirely (commands stay palette-reachable — menu-only, no feature removal); (3) **⌘N = ritemark.newDocument**. Also shipped without needing decisions: Info.plist version stamp (#175, Finder showed 1.117.0), chat file links open in Ritemark + writer-voice Project empty state (#176), folderless Home parity with recent folders (#177), and the agent-write staleness root-cause fix — level-triggered reconcileWithDisk against a last-sent-to-webview hash (#178). AI disclosure show-once behavior verified intact across full restart. | Jarmo + test-pass execution |
| 2026-08-04 | Sprint 107 R4 ("Claude is ready" welcome card removal) shipped EARLY and standalone on Jarmo's direct order — the sprint queue had reached 107, no sprint-107 branch existed yet. Webview-only: `AISidebar` now routes through a pure `sidebarGate()` (unit-tested), the ready-with-no-conversation state goes straight to the composer, `dismissWelcome()` bookkeeping fires automatically (live-verified: `ritemark.ai.hasSeenClaudeWelcome` persisted without a click), and `SetupWizard`'s unreachable isReady branch is gone. Remaining Sprint 107 scope (R1 patch 013, R2 daemon consent, R3 tab healer) unchanged. | Jarmo + Sprint 107 R4 early ship |
| 2026-08-04 | Jarmo ordered Home FIRST in the Activity Bar ("home peab olema esimene") + a proper design review of the Home view — explicit shell-tier escalation approval for the pinning. Patch 002 now pins `ritemark-home` at `effectiveOrder` −1 (before Explorer) in `viewsExtensionPoint.ts`; Home view design pass replaced placeholder unicode glyphs with inline SVG icons (Phosphor-style, currentColor) and Indigo-Editorial CTA/row polish. Live-verified in a fresh dev profile: Home is the first Activity Bar tab, open-recent flow opens the doc in the Ritemark editor; marketing shot `1-8-6-home-launcher.png` replaced with the verified capture. | Jarmo + Sprint 106 follow-up |
