...
- [x] **Closeout (release skill Step 10, 2026-09-14):** published assets verified against GitHub digests, the canonical feed and the build-time sidecars (all equal); local-only evidence kept in `evidence/`; build output cleared, worktree classified `REVIEW` and removed with `worktree-hygiene.mjs --clean` on Jarmo's go. Details in `evidence/closeout.md`.
