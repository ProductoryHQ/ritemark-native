...
- **macOS arm64**: **blocked.** In a fresh `create-release-worktree.sh` tree at `640e2aac`, `release-preflight.sh` passed (1 expected warning) but `build-prod.sh` Step 1 refused: the clean `npm ci` + `vite build` bundle differs from the committed `media/webview.js` by 11 two-byte `\r` escapes inside `className` template literals (bundle committed from a Windows autocrlf checkout in `e4321178`). Fix candidates: [#267](https://github.com/ProductoryHQ/ritemark-native/pull/267) (LF rebuild + `.gitattributes`, product-bytes change → all three platforms rebuild from the new commit) or [#268](https://github.com/ProductoryHQ/ritemark-native/pull/268) (harness-only `build-prod.sh` tolerance, x64/Windows stand). Jarmo decides.
...
| macOS arm64 | — | blocked (#267 / #268) | pending | open |
