...
- [x] Twenty rapid type → save → continue-typing cycles keep the view quiet, retain every visible suffix, and end byte-equivalent to disk; deterministic ordered receipts cover successful ordinary and format-on-save disk content, retire stale confirmed receipts after unmatched later observations, preserve saves completed during an older read, and keep canceled/unconfirmed or unknown snapshots on the true-conflict path (2026-08-31 RC correction).
