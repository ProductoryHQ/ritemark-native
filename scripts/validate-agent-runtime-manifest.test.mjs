...
test('rejects an incomplete platform matrix', () => {
  const { manifest, packageJson, packageLock } = fixture();
  manifest.runtimes = manifest.runtimes.filter((runtime) => !(runtime.agent === 'codex' && runtime.platform === 'win32'));
  const errors = validateAgentRuntimeManifest(manifest, packageJson, packageLock);
  assert.ok(errors.some((error) => error.includes('manifest must contain 9 runtime rows')));
  assert.ok(errors.some((error) => error.includes('codex targets must be')));
});
